    ...
    public void rollback(TransactionId tid)
        ...
        synchronized (Database.getBufferPool()) {
            synchronized(this) {
                ...
                if (startOffset == null) {
                    throw new NoSuchElementException("Transaction " + tid.getId() + " does not exist or has no log records.");
                }
                raf.seek(startOffset);
                try {
                    // wrap in a try catch exception 
                    // catch an end of file exception
                            ...
                            switch (recordType) {
                                case UPDATE_RECORD:
                                    ...
                                    if (transactionId == tid.getId()) {
                                        HeapFile hf = (HeapFile) Database.getCatalog().getDatabaseFile(beforePage.getId().getTableId());
                                        Database.getBufferPool().discardPage(beforePage.getId());
                                        hf.writePage(beforePage);
                                    }
                                    break;
        ...
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                if(!recoveryUndecided) {
                    ...
                    long startOffset = checkpointOffset != NO_CHECKPOINT_ID ? checkpointOffset : 0; // read from start if no checkpoint
                    raf.seek(startOffset);
                    ...
                    try {
                        while (raf.getFilePointer() < raf.length()) {
                            ...
                            switch (recordType) {
                                ...
                                case COMMIT_RECORD:
                                    ...
                                    break;
                                case UPDATE_RECORD:
                                // check if the tid gotten from raf is already completed or not
                                // if not, read page data of raf and write the page to the database
                                // break
                        ...
                        // Log and/or handle unexpected end of file
                        System.err.println("Unexpected end of file during recovery.");
                        e.printStackTrace();
                    }
                }
                // some code goes here
                // seek to raf = 0
                // read the checkpoint offset from raf file [first long stored]

                // set that tracks all of the completed actions 
                // check if checkpoint offset is != -1, there is an issue there 
                // check if checkpoint record type -> throw an exception 

                // seek again to raf.length of raf 
                // go through a while loop, offset > size of a long
                // go through raf logic and get type of the record 
                // do something for each record type 
                // begin, checkpoint dont do anything and break 
                // commit record -> add to set 
                // update record -> check if the tid gotten from raf is already completed or not
                    // if not, read page data of raf and write the page to the database
                    // break
                    // if not throw exception 
                // abort record -> add tid into set, break 
